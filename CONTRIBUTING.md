...
**Замеры веса в документации — с официальной сборки Node.** Цифры gzip
в `README.md` и на страницах `docs/` считает `npm run sync-sizes` по `dist`,
и они зависят от zlib внутри Node: официальные сборки 22 и 24 (их ставит CI)
несут zlib 1.3.x, Homebrew-Node на macOS жмёт системным zlib 1.2.x — тот же
файл выходит на несколько десятков байт легче, и на границе округления
цифра в тексте меняется. Эталон в дереве — с официального Node; на другом
zlib сверка без ключа (`npm test`) проходит с допуском 50 Б, а `--fix`
допуска не знает и пишет точный замер. Поэтому на Homebrew-Node
`npm run build` (он вызывает `sync-sizes --fix`) может переписать цифры
в нескольких файлах — эти правки не коммитьте, а верните
`git checkout -- README.md docs/`. Официальную сборку проще всего держать
через [fnm](https://github.com/Schniz/fnm) или nvm.
...
1. **Заведите issue** до начала работы, если правка не тривиальна. Опишите
   ...
   Заводить — на GitVerse: копия на GitHub только зеркалит выпуски, задачи
   и запросы на слияние там выключены.
...
Правило появилось, когда 70 % ядра слоя обслуживало меньшинство
