...
| **`griffincss-ui`** | Компоненты интерфейса: кнопки, поля, флажки, карточки, таблицы, сообщения, плашки, аватары, навигация, меню, вкладки, аккордеон, модальное окно, выдвижная панель, подсказки, пагинация, прогресс, спиннер, заглушки, тосты, шаги, пустое состояние. Плюс опциональный слой GriffinJS: слайдер, галерея, лайтбокс, параллакс, мегаменю, комбобокс, ползунок, сортируемая таблица | <!--gr:size:ui-->12,2 КБ<!--/gr:size:ui--> CSS + <!--gr:size:js-ui-->2,9 КБ<!--/gr:size:js-ui--> JS (+ <!--gr:size:griffinjs-->19,2 КБ<!--/gr:size:griffinjs--> GriffinJS по желанию) |
...
без виджетов слоя): `griffincss-ui.css` 12 541 → 7 546 Б gzip с пресетом
(6 868 без него — но без тостов); `griffinjs.css` 2 880 → 1 596 Б.
...
Пресет не знает про ось оформления: значения `data-gr-style`, которые
ставятся не в разметке, а из настроек или рантаймом
(`Griffincss.theme.style('compact')`), в файлах не встречаются, и блоки
этих стилей уходят вместе с неиспользованными классами. Перечислите их
в `--safelist` явно: `--safelist airy,compact`. Отчёт скрипта называет,
что по оси оставлено и что выброшено, — сверяйте с настройками темы.
