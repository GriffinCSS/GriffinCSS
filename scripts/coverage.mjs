...
//
// Библиотека стоит на каскадных слоях, контейнерных запросах и `:has()`.
// Порог браузеров из-за этого выше, чем «последние версии Chrome
// и Safari», и он обязан
...
//
// Колонка «где» — не украшение: у `subgrid` порог заметно ниже прочих,
// и читателю важно, за что именно он его платит.
const FEATURES = [
  {
  ...
  // subgrid ниже всех остальных, и молчать об этом нельзя: одна утилита
  // ядра опускает планку на три пункта против слоёв. Деградация штатная —
  // правило отбрасывается, и .gr-subgrid остаётся обычной сеткой, — но
  // читатель узнаёт об этом из таблицы, а не из чужой вёрстки.
  {
    id: 'css-subgrid',
    title: 'subgrid',
    where: 'только утилиты .gr-subgrid и .gr-subgrid-rows ядра',
  },
];
