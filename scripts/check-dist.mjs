...
// Этап 49: 9 189 → 9 223 Б вслед за рантаймом темы (1 355 → 1 393 Б):
// исключение .gr-theme-keep из гашения переходов и событие смены после
// снятия гашения — фидбек темы griffin по 0.26.0, п. 3. Потолок 9,1 КБ.
const BUNDLE_BUDGET = Math.round(9.1 * 1024);
...
const CSS_BUDGETS = [
  ...
  // Этап 50 (фидбек темы griffin по 0.26.1): 12 541 → 12 604 Б, потолок
  // 12,4 КБ. ПОКУПКА трёх частей: образец-ссылка — выбранное
  // по aria-current, фокус на самом образце и своё снятие подчёркивания
  // у a.gr-swatch-tag (+25 Б: значение варианта — ссылка на его адрес,
  // вид состояний был только у радио, а подчёркивание из слоя темы
  // доходило до плашки — у остальных компонентов-ссылок оно своё); токены
  // --gr-swatch-size / --gr-swatch-radius и картинка на всю площадь
  // образца (+33 Б: фото варианта не собиралось — шкалы размеров
  // в утилитах нет, а поля плашки теме из подслоя не перебить);
  // a.gr-page[aria-current] (+5 Б: текущая страница-ссылка в режиме
  // для слабовидящих читалась 1,00 : 1).
  ['ui', kbBudget(12.4), UI],
...
//     Набор слайдера, Этап 49 (2026-09-22): 7 808 → 7 895 Б — выросло
//     ядро, 2 733 → 2 820 Б (+87). ПОКУПКА: старт слоя под defer — ядро
//     стартовало при выполнении собственного файла, раньше defineWidget
//     модулей дальше в бандле, и ни один виджет не поднимался (фидбек темы
//     griffin по 0.26.0, п. 1). Отложенный тег ждёт DOMContentLoaded
//     со страховкой readystatechange, тег в готовом документе стартует
//     микрозадачей. Потолок набора 7,8 КБ; ядро в своих 2,8 КБ.
const GRIFFIN_CORE_BUDGET = Math.round(7.8 * 1024);
